...
if (searchKey != null) {
	...
	prodList = productService.getAllProductsBySearchKey(searchKey, currentPage);
	totalPages = (int) Math.ceil((double) productService.productCountBySearchKey(searchKey) / 20);
} else if (catId != null && !(catId.trim().equals("0"))) {
	prodList = productService.getAllProductsByCategoryId(Integer.parseInt(catId.trim()), currentPage);
	message = "Hiển thị kết quả cho \"" + categoryService.getCategoryName(Integer.parseInt(catId.trim())) + "\"";
	totalPages = (int) Math.ceil((double) productService.productCountByCategoryId(Integer.parseInt(catId.trim())) / 20);
} else {
	prodList = productService.getProductsByPage(currentPage);
	message = "Tất cả hàng hóa";
	totalPages = (int) Math.ceil((double) productService.productCount() / 20);
}

if (prodList != null && prodList.size() == 0) {
	message = "Không có sản phẩm nào cho \""
	+ (searchKey != null ? searchKey : categoryService.getCategoryName(Integer.parseInt(catId.trim()))) + "\"";
	prodList = productService.getProductsByPage(currentPage);
	totalPages = (int) Math.ceil((double) productService.productCount() / 20);
}log(""+prodList.size());
%>
			...
			<div class="col">
				<div class="card h-100 px-2 py-2">
					<div class="container text-center">
						<img src="http://localhost:8080/DoAnCuoiKy/Product_imgs/<%=p.getProductImages()%>"
							class="card-img-top m-2"
							style="max-width: 100%; max-height: 200px; width: auto;">
						<div class="wishlist-icon">
							<%
							if (u != null) {
								if (wishlistService.getWishlist(u.getUserId(), p.getProductId())) {
							%>
							<button
