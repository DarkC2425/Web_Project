...
<%@page import="com.shop.service.Impl.OrderedProductServiceImpl"%>
<%@page import="com.shop.service.Impl.OrderServiceImpl"%>
...
OrderedProductServiceImpl ordProdService = new OrderedProductServiceImpl();
		...
		<div class="container-fluid">
			<table class="table table-hover">
				...
				for (Order order : orderList) {
					List<OrderedProduct> ordProdList = ordProdService.getAllOrderedProduct(order.getOrderId());
					for (OrderedProduct orderProduct : ordProdList) {
				%>
				<form action="../../UpdateOrderController?oid=<%=order.getId()%>" method="post">
			<tr>
					<td class="text-center"><img
						src="http://localhost:8080/DoAnCuoiKy/Product_imgs/<%=orderProduct.getImage()%>"
					...
					<td><%=userService.getUserName(order.getUserId())%><br>Số điện thoại: <%=userService.getUserPhone(order.getUserId())%><br><%=userService.getUserAddress(order.getUserId())%></td>
					<td><%=order.getDate()%></td>
